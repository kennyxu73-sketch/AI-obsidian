...
|                                      | **S2b: 企微 Bot（通道）** | 通知推送、Inbox 收录、只读查询；与积压提醒同源              | 企业微信作为提醒与收录入口                       | **不得绕过** SSOT 审批；收录默认进队列、不直写 SSOT | 三条 MVP 可验收；操作可追溯；无未审批直写 SSOT     |
...
- **P1 -> P2**：分批导入稳定、审批积压可控、检索质量达标、周运营闭环稳定；**企微等外部通道不得绕过 SSOT 审批规则**（与 P0「未审批写入 = 0」一致）。
...
gantt
    ...
    企微通知与 Inbox MVP          :p1s2w, after p1s1, 4d
    section S3: 语义检索
...
**P1 甘特说明**：条块为逻辑先后；**S3（检索）与 S5（周闭环）等在人力允许时可与前排期并行**，不必严格串行。最小可交付切片以 [[AI-OB_开发原则与P1精进计划]] 为准。
