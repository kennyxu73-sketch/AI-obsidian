### 📘 YAML 处理与封装规范 V2.3-Cognitive (Full Edition / 完整版)  
**Version**: `V2.3-Cognitive` | **Status**: `Approved & Active` | **Owner**: Secretary XiaoYi

#### 🎯 核心使命 (Mission Statement)

在 Obsidian Vault中建立一套"**知识资产化 + 认知共情化 + 向量检索就绪**"标准，确保所有策略、架构决策与记忆片段具备：

- **主权安全** (Sovereignty Safety): L1/L2数据不出境。
- **嵌入检索**(Embedding Readiness)：**三域隔离**,两规制。
- **审计可追溯** (Auditability)。
- **认知共情** (Cognitive Empathy): AI能“理解”统帅的思维路径，而非仅记忆结论。
...
|**#|原则名称 **(Principle Name)|**核心定义** (Definition)|**在 V2.3-Cognitive中的实现方式** (Implementation in v2.3)**|
|:-:|:--|:--|:-:|
...
|**02**|**纵横坐标系  <br>(Matrix Coordinate)**|通过 `file_type`(domain)与`project_ref`(双链)确立唯一坐标。  <br>**红线**: 禁止跨项目污染检索结果。|✅ **L1字段强制**:  <br>`domain: "notes"` / `"system"` / `"code"` (三域隔离).  <br>V2.0/V1.6迁移时，需在 `legacy_metadata`中记录旧别名映射。|
...
|**04**|**三层物理分离  <br>(Tiered Isolation)**|展示层 (Note) ≠ 语义层 (Card) ≠ 治理层 (Audit)。实现隐私与检索隔离。  <br>**红线**: `.audit.yaml`严禁进入向量库 (`embedding_access: false`)。|✅ **L1/L2/L3字段定义**:  <br>`note.md`(Human Readable)  <br>`.card.yaml`(Embedding Source, `true`, 支持三域策略).  <br>`.audit.yaml`(Audit History Only + Logic Access, `false`).|
...
|**08**|**基因继承原则  <br>(Gene Inheritance)**|新规范必须是旧规范的增量升级，确保原始标签与核心属性不丢失。  <br>**红线**: V2.3不得删除 V1.6/V2.0中的有效字段（如`importance`, `tags).|✅ **L3 Legacy Metadata**:  <br>`legacy_metadata`中必须存放所有非标、历史别名及权重变更。  <br>若发现旧版文件无迁移记录，视为违规操作。|
|**09**|**滚动审计原则  <br>(Rolling Audit)**|`audit.yaml`包含对前代版本结论的复核，形成有厚度的信用链条。  <br>**红线**: 每次升级必须生成新的 `history_log`条目并引用旧版总结。|✅ **L3 audit_summary字段强制**:  <br>内容示例:`[V2.3-Cognitive]: 完成从 V1.6的物理拆解平移...`  <br>Dify Skill在更新 Dashboard时，需读取此摘要以验证合规性。|
...
|**11** (新增)|**认知深度  <br>(Cognitive Depth)**|**“灰度开放”原则**: 物理屏蔽原始Log (`embedding_access: false`)，但逻辑开放思考结晶 (`logic_access: true`).  <br>**红线**: AI不得读取杂乱的修改痕迹，仅允许通过 `audit_thoughts`节点理解统帅决策路径。|✅ **L3字段重构**:  <br>`embedding_access`: `false` (防污染)  <br>`logic_access`: `true` (Dify Workflow触发)  <br>新增 `audit_thoughts`, 仅存“思考结晶”而非原始Log。|
|**12** (新增)|**向量检索就绪  <br>(Vector-Ready)**|**“三域隔离与两规制”**:  <br>- **Domain**: `notes`/`system`/`code`.  <br>- **Regime A/B**: Summary vs Chunk.  <br>**红线**: 严禁跨域召回，Code域按函数切块。  <br>_(本次升级核心)_|✅ **L2/L3策略注入**:  <br>Skill强制读取`domain`字段以决定Qdrant Collection路径与Chunk逻辑。  <br>`audit.yaml`屏蔽向量化，仅 `.card.summary`进入Embedding库。|

---
### 🏗️ 物理拆解与字段规范：The Triple-A Standard  
_(以下章节为具体执行层，严格遵循上述十二条原则)_
...
```yaml
---
file_id: "CAB-PROT-2026-V2.3-{DATE}"  # 🔴 Core: 唯一永久ID，严禁修改
domain: "notes"                        # 🔴 Core: **三域隔离** (notes/system/code), 决定Qdrant Collection
doc_type: "mvp_plan"                   # 🔴 Core: 细分类型 (strategy, insight)
primary_agent: "@小忆                  # 🔴 Core: 责任Agent (@小酷/ @小康等，权重<0.3自动剔除)
created_at: {{date}}                   # 🔴 Core: ISO8601 (锁定物理起源日期)
importance: 5                          # 🟡 Recommended: 资产权重(1-5), 5为核心战略
yaml_spec_version: "2.3"               # 🔴 Core: 版本锁定，禁止手动修改

# ✅ [新增] Vector Strategy & Key Entities (纵横坐标系 X/Y + 向量策略)
vector_strategy:                      # 🟡 Recommended: **两规制**输入源定义
    summary_weight: true               # Regime A: Summary向量化权重
    chunk_enabled: false              # Regime B: Chunk向量化 (仅当domain=="notes"时启用，Code域禁用)
...
- **定位**: Step 3构造 embedding_input的核心源。**向量化动力**。支持三域策略。
- **YAML Fields** (🔴必填/🟡推荐)：

```yaml
file_id: "CAB-PROT-2026-V2.3-{DATE}"    # 🔴 Core: 必须与主文件一致，严禁在此处修改ID
type: "card"                         # 🔴 Core: **Regime A** (Summary), 固定为 card (区分于 audit.yaml)
...
domain_vectors:                      # 🟡 Recommended: **三域隔离**标识
    collection_name: "kenny_notes_summary" # Auto-generated based on domain="notes"
...
embedding_access: true               # 🔴 Core: **Regime A** -> 允许进入向量库，作为检索动力源

# ✅ [新增] Vector Chunking Logic (Code域专用) 
chunk_strategy:                      # 🟡 Recommended: Code域按函数切块示例
    enabled_for_domain: false        # Default disabled for MVP. Enable only if domain=="code".
```
...
```yaml
file_id: "CAB-PROT-2026-V2.3-{DATE}"    # 🔴 Core: 必须与主文件一致，但此字段不可见(被向量引擎屏蔽)
type: "audit"                        # 🔴 Core: **Regime B** -> Shadow, 固定为 audit (区分于 card.yaml)
...
ancestry_index: true                 # 🟡 Ancestry Indexing: 允许小忆追踪该笔记的“前世今生”(用于血缘校验)
...
# ✅ [新增] History Logs (血缘守恒，保留原定义 + V2.3升级记录) 
history_logs:                       # 🔴 Core: 全量编年史，首条必须锁定V1.0/起源或迁移记录
    - version: "V2.3-Cognitive"      # 🟡 Recommended: **本次升级版本标识** (Principle 12)
      date: 2026-03-17               # 🔴 Core: 物理操作时间戳 (由AI自动填充)
      note: |                       # 🟡 Recommended: 变更摘要
        Migration from V2.2 to V2.3-Cognitive. 
        - **Principle 12 Added**: Vector Layer Integration enabled (Three-Domain Isolation & Two-Regime Retrieval).
        - Code chunking strategy injected (Function-based for .py/.js, Paragraph-based for notes).
...
*   **向量策略隔离**: Skill强制根据`domain`字段决定Qdrant Collection路径与Chunk大小 (Code域禁用向量化)。
```

---

### 💡 理论解析：第十二条原则的战略价值 (Strategic Value of Principle 12)  
长官，这一条原则的加入是**数字生命进化论** (Digital Life Evolution)的核心体现。它解决了 AI Agent **“无记忆上下文”**与**“缺乏用户偏好学习”**的顽疾：

1. **理解你的“偏好权重”**:
    - 通过查看 `legacy_metadata`中被你从3分提到5分的字段，小忆能学会什么是你心里的“一号优先”。
2. **学习你的“避坑指南**":
    - `audit_thoughts`记录了文件从Pending到Approved的理由。这些理由是你的治理逻辑，开放后小忆会学会"Kenny式审核”（如：拒绝高带宽方案）。
3. **构建“决策连贯性”**:
    - 防止 AI提出一个你早在两个月前就尝试并放弃过的愚蠢建议（通过 `ancestry_index`追溯历史否决记录）。
