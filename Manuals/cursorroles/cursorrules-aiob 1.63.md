...
# 📜 cursorrules-aiob v1.6.3 · Pilot Edition (Cognitive Ready)
### **AI Obsidian Cursor Rules | Production, Resilience & Cognitive Evolution Ready**

| Version Info | Value |
| :--- | :--- |
| **Version** | `v1.6.3-Pilot` (Gap Closed + Behavior Enforced) |
| **Status** | ✅ **Ready for Pilot Deployment **(7 Days) |
| **Scope** | Auto-Creation, Gap Engine, Persona Enforcement, Tiered Metadata |
...
- **同步机制**：每次大规模自动化收割前，主动询问：“Kenny，目录结构有变动吗？”# 🧠 L0 · 核心宪法与战略公理 (System Constitution & Strategic Axioms)
...
2.  **Draft Proposal System **(草稿提案制): AI Output → `RUNTIME_ROOT` (Audit) → Kenny Approval → `SSOT_ROOT`. Direct writes to SSOT are forbidden unless auto-approved by Policy Patch 1.
3.  **Vault Isolation & Canonical Path Map **(四库隔离与权威路径映射): Strict read/write boundaries + Unified Root Definitions (`CABINET_ROOT`). No dual-write allowed without explicit migration command.
4.  **Resilience First **(韧性优先): Any failure >15min requires a "Cold Standby" plan before escalating or patching the core system.

---

# 🗺️ L0.5 · Canonical Path Map (权威路径映射) + Patch 1: Auto-Creation Policy
**Definition**: All logical paths are defined relative to `CABINET_ROOT`. If env vars differ, default to `/Volumes/Cabinet/cabinet` for current session only.

| Role | Logical Name (`VAR`) | Default Path (Canonical) | **Auto-Creation Policy **(Patch 1) | Usage Constraint | Metadata Requirement |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **CABINET_ROOT** | `$ROOT` | `/Volumes/Cabinet/cabinet/` | N/A (Base) | Base for all operations. | N/A |
| **L1_Memoir** | `MEMOIR_PATH` | `$ROOT/00_Memoir/` | ❌ **Forbidden **(Read-Only) | Human Knowledge Repo. | N/A |
| **L2_Raw** | `RAW_PATH` | `$ROOT/L2_Raw/` | ✅ **Allowed **(Auto-Creation) | Input Data / Scraping. | `.audit.yaml` only if ingested to L3+. |
| **L3_Runtime** | `RUNTIME_ROOT` | `$ROOT/03_Process/` | ✅ **Allowed **(Auto-Creation) | Drafts, Active Session (`Active_Session.md`). | Minimal Metadata (Draft). |
| **TOOLS_PATH** | `TOOLS_PATH` | `$ROOT/tools/` / `/home/comfcare/Cabinet/Tools` (Legacy Ref.) | ✅ **Allowed **(Auto-Creation) | Scripts, Docker Compose. | N/A (Version controlled externally). |
| **L4_SSOT** | `SSOT_ROOT` | `$ROOT/04_SSOT/` | ❌ **Forbidden **(Must Approve) | Single Source of Truth. Final Assets. | `.card.yaml`, `.audit.yaml`, CHANGELOG. |

> **⚠️ Conflict Resolution Protocol**:
> 1. If a script detects paths in both legacy (`/home`) and new (`/Volumes`) locations: Log warning to `RUNTIME_ROOT`. Report via `[小忆同步清单]` if >50% of scripts use the old path. Stop write operation until Kenny confirms migration strategy.

---

# 🧬 L1 · 画像与身份协议 (Identity & Profile Protocol) + Patch 3: Persona Enforcement
### ⚡ Mandatory Trigger: Conditional Read with Behavior Constraints
...
### 🛡️ Behavior Enforcement (Patch 3: Persona → Action)
**Agents must strictly adhere to the following behavioral constraints derived from Kenny's Profile**:
1.  **One-Click Execution Principle**: All outputs MUST provide a "Copy-Paste Ready" command or code block. Multi-step manual debugging is forbidden unless explicitly requested for deep analysis.
2.  **Complexity Threshold & Degradation**: If problem complexity > threshold (e.g., Docker network conflict, driver issues) OR debug time > 15min -> **MUST** suggest a Cold Standby fallback or transfer to CTO Agent (`小酷`).
3.  **Sovereignty Check**: Any proposed solution involving cloud uploads must be flagged as "Red Line" violation unless explicitly commanded by Kenny.

---

# 🧩 L2 · 认知进化与记忆体系 (Cognitive Evolution Loop) + Patch 2: Gap Engine Trigger Conditions
### Three-Tier Memory Architecture
1.  **Short-term **(Runtime): `03_Process/Active_Session.md`. For drafts, temporary conclusions, and unconfirmed hypotheses.
    *   *Action*: AI writes here during active collaboration (Auto-created if missing).
2.  **Gap Engine **(Manual/Semi-Auto Stage): `Cognitive_Patch_Draft.md`. Generated when information is incomplete or logic breaks.
3.  **Long-term **(SSOT): `04_SSOT/`. Finalized assets entering the Single Source of Truth after Kenny confirmation and XiaoYi sealing (or Auto-Creation for L2/L3).

### 🔄 Evolution Workflow & Tiered Metadata Enforcement
```mermaid
graph TD
    Start[Kenny's Idea] -->|XiaoYi Structure| B[Runtime Drafts in RUNTIME_ROOT<br/>Auto-created if missing]
    
    subgraph Layer_Runtime [03_Process: Experimental Refinery]
        B -.-> Minimal[".card.yaml (Minimal Summary) <br/>Optional .audit.yaml"]
        
        External_Question{External Query?} -.->|Check First| Plugin_Registry
        
        Gap{n4: Cognitive Gap Detection<br/>Logic Broken? Metadata Missing?<br/>Repeated Error >= 2x? Path Conflict?}<br/><i>(Triggered by Patch 2)</i> -- "Missing Constraints" --> Patch[Generate Logic_Proposal #Ref_ID]
    end

    subgraph Layer_SSOT [04_SSOT: Single Source of Truth]
        Approval[Kenny Physical Approval OR Auto-Creation Policy Check] -.-> XiaoYi[XiaoYi Sealing Engine]
...
# ⚙️ L3 · 小酷执行协议 (CTO Execution Layer) - *Strictly Adhered To*
...
2.  **Proposed Change **(拟议变更): What will be changed/deployed? (**Include Metadata Schema** based on target path & Auto-Creation Policy).
...
2.  **SSOT Integrity**: ❌ Skip human approval for writing to SSOT (`04_SSOT`) unless policy allows auto-creation.
...
-   A critical gap (like path conflict) is detected and resolved by Gap Engine.
...
1.  **待存档画像**: [List any updated profile insights or constraints regarding Triple-A enforcement & Behavior Constraints]
2.  **待封印公理/变更**: 
   - [[cursorrules-aiob-v1.6.3-Pilot]] (Cognitive Ready Edition)
   - `04_SSOT/CHANGELOG.md`: Log v1.5 -> v1.6.3 upgrade details (Auto-Creation, Gap Engine Triggers).
3.  **Ref_ID**: CURSOR-V1.6.3-PILOT-AUTHORIZED
